    ...
    A: FindUserCredentialDao + FindUserSecretDao + AuthenticateUserDao + ChangePasswordDao,
{
    ...
    pub async fn call(&self, login: String, password: String) -> Result<Session, AppError> {
        ...
        println!("3 - user_id - {}", credentail.user_id.clone());
        // let secret = self.repo.find_user_secret_by_user_id(credentail.user_id).await.unwrap().unwrap();
        println!("4 - digest `{}`", secret.password_digest);
...
mod tests {
    ...
    #[tokio::test]
    async fn second_authenticate_user_command() {
        // Given
        let postgres_container = postgres::Postgres::default()
            .with_tag("18.1-alpine")
            .start()
